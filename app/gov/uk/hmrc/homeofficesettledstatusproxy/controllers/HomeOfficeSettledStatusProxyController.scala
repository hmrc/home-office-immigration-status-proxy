...
class HomeOfficeSettledStatusProxyController @Inject()(
  ...
  def entities: Action[AnyContent] = Action.async { implicit request =>
    Future.successful(Ok(toJson(HomeOfficeSettledStatusProxyModel("hello world", None, None, None))))
  }

  def entitiesByUtr(utr: Utr): Action[AnyContent] = Action.async { implicit request =>
    Future.successful(Ok(toJson(HomeOfficeSettledStatusProxyModel(s"hello $utr", None, None, None))))
  }
