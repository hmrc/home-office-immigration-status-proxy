...
trait JsonMatchers {
  ...
  def haveProperty[T: Reads](name: String, matcher: Matcher[T] = null)(
    ...
    (obj: JsObject) => (obj \ name).asOpt[T] match {
      case Some(value) =>
  ...
  def havePropertyArrayOf[T: Reads](name: String, matcher: Matcher[T] = null)(
    implicit classTag: ClassTag[T]): Matcher[JsObject] =
    (obj: JsObject) => (obj \ name).asOpt[JsArray] match {
      case Some(array) =>
        if (matcher != null)
          array.value
            .map(_.as[T])
            .foldLeft(MatchResult(matches = true, "", ""))((a: MatchResult, v: T) =>
  ...
  def notHaveProperty(name: String): Matcher[JsObject] =
    (obj: JsObject) => (obj \ name).asOpt[JsValue] match {
      case Some(value) =>
        MatchResult(
          matches = false,
  ...
  def eachElement[T](matcher: Matcher[T]): Matcher[Seq[T]] = (left: Seq[T]) => left.foldLeft(MatchResult(matches = true, "", ""))((a: MatchResult, v: T) =>
    if (a.matches) matcher(v) else a)


  def oneOfValues[T](values: T*): Matcher[T] = (left: T) => MatchResult(
